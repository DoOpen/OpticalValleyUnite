...
@property (nonatomic, strong) NSMutableArray *indexPathsSelectedItems;
@end
...
//加载图片
- (void)setupAssets {
    [self.indexPathsSelectedItems removeAllObjects];
...
- (BOOL)shouldSelectAsset:(ALAsset*)asset {
    ...
    if (!vc.multipleSelection && self.indexPathsForSelectedItems.count==1) {
        NSInteger index = [self.assets indexOfObject:self.indexPathsSelectedItems[0]];
    ...
    return (selectable && self.indexPathsSelectedItems.count < vc.maximumNumberOfSelection);
}

- (void)didSelectAsset:(ALAsset*)asset {
    [self.indexPathsSelectedItems addObject:asset];
    
    BoPhotoPickerViewController *vc = (BoPhotoPickerViewController *)self.my_delegate;
    vc.indexPathsForSelectedItems = self.indexPathsSelectedItems;
...
- (void)didDeselectAsset:(ALAsset*)asset {
    [_indexPathsSelectedItems removeObject:asset];
...
- (NSMutableArray *)indexPathsSelectedItems {
    if (!_indexPathsSelectedItems) {
        _indexPathsSelectedItems = [[NSMutableArray alloc] init];
    }
    return _indexPathsSelectedItems;
}
@end
